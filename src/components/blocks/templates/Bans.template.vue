    ...
    <template v-if="result.error == ''">
      ...
      <div class="output__body">
        <div class="output__table-active">
          <div class="output__body-title">Список активных банов</div>

          <template v-if="!result.result.active">
            <div class="output__subtitle">Активный бан не найден</div>
          </template>
            ...
            <div class="output__body-active">
              ...
              <div v-else class="output__body-active-list-mobile card">
                <div class="output__body-active-item-mobile">
                  ...
                  <div class="output__body-active-line-mobile">
                    Истекает: {{ result.result.active.expires == 0 ?  'Забанен пермаментно' : 'Через ' + result.result.active.expires}}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="output__history">
          <div class="output__body-title">Список банов</div>

          <template v-if="result.result.history == false">
            <div class="output__subtitle">баны не найдены</div>
          </template>
          ...
          <template v-else>
            <div class="output-history__list">
              <div class="output-history__item card" v-for="(item, index) in result.result.history">
                ...
                <div class="output-history__item-line">Дата бана: {{item.pastCreated}}</div>
                ...
                <div class="output-history__item-line">Время бана: {{item.expired == 0 ? 'Перманентный бан' : item.expired}}</div>
                <div class="output-history__item-line">
                  Кто банил:
                  <RouterLink :to="`/?act=bansadm&login=${item.pastActor}`">
                    {{item.pastActor}}
                  </RouterLink>
                </div>
                <div class="output-history__item-line">Дата разбана: {{item.created}}</div>
                <div class="output-history__item-line">
                  Кто размутил:
                  <RouterLink :to="`/?act=bansadm&login=${item.actor}`">
                    {{item.actor}}
                  </RouterLink>
                </div>
              </div>
